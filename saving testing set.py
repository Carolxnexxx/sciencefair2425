...
for drug in tqdm(matching_drugs, desc="Processing Selected Drugs"):
    ...
    # Capture the test set data (drugs and corresponding values)
    test_set_data = []
    for idx in X_test.index:
        test_set_data.append({"Drug": drug, "Cell Line Index": idx, "True Value": y_test.loc[idx]})
    
    # Save the test set for the current drug to a CSV file
    test_set_df = pd.DataFrame(test_set_data)
    test_set_df.to_csv(f"test_set_{drug}.csv", index=False)
    print(f"Testing set for {drug} saved to test_set_{drug}.csv")
